...
//네비바 숨김 하위페이지까지
  ...
  const isHidden = (exactArr, prefixArr) =>
    exactArr.includes(pathname) || prefixArr.some(p => pathname.startsWith(p));

  const hideHeader = isHidden(hiddenHeaderExact, hiddenHeaderPrefixes);
  const hideNav = isHidden(hiddenNavExact, hiddenNavPrefixes);

  const TITLE_RULES = [
    { test: p => p.startsWith("/explore/doc/"), title: "등기부등본//안뜨는 화면" },
    { test: p => p.startsWith("/explore"),      title: "매물 분석" },

    { test: p => p.startsWith("/contract"),     title: "계약 도우미//안뜨는 화면" },
    ...
    { test: p => p.startsWith("/my/cleaning"),  title: "이사/청소 서비스//제외" },
    { test: p => p.startsWith("/my/legal"),     title: "법률 연계//제외" },
    { test: p => p.startsWith("/my"),           title: "마이페이지" },

    { test: p => p === "/" || p.startsWith("/home"), title: "홈" },
  ];
