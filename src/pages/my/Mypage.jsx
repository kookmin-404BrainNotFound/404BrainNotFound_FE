...
export default function MypageLayout({ username = "회원1" }) {
  ...
  return (
    <div className="p-5 space-y-6">
      ...
      <button onClick={goToReports} className="bg-[#D9D9D9] rounded-md p-5 text-left shadow-sm hover:bg-gray-200 w-full">
        <div className="text-lg font-medium">내 리포트 모아보기</div>
        <div className="text-sm font-medium">14건</div>
      </button>

      <div className="grid grid-cols-2 gap-3">
        <button onClick={goToCleaning} className="bg-[#D9D9D9] rounded-md aspect-[5/4] flex items-start justify-start p-5 text-md font-medium text-left shadow-sm hover:bg-gray-200">
          이사/청소 서비스<br /> 신청하기
        </button>

        <button onClick={goToLegal} className="bg-[#D9D9D9] rounded-md aspect-[5/4] flex items-start justify-start p-5 text-md font-medium text-left shadow-sm hover:bg-gray-200">
